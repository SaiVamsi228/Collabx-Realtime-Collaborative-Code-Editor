...
const AuthPage = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = auth.onAuthStateChanged((user) => {
      console.log("AuthPage: onAuthStateChanged fired, user:", user?.uid, "loading:", loading);
      if (user) {
        console.log("AuthPage: User authenticated, redirecting to /dashboard", user.uid);
        navigate("/dashboard", { replace: true });
      }
    });

    return () => {
      console.log("AuthPage: Cleaning up onAuthStateChanged listener");
      unsubscribe();
    };
  }, [location, navigate]); // Removed 'loading' from dependencies to avoid remount issues
  ...
  const handleGoogleLogin = async () => {
    ...
    try {
      ...
      console.log("AuthPage: Resetting loading state after Google auth");
      setLoading(false);
    }
  };
  ...
  const handleGitHubLogin = async () => {
    ...
    try {
      ...
      console.log("AuthPage: Resetting loading state after GitHub auth");
      setLoading(false);
    }
  };
  ...
  const handleEmailSubmit = async (e) => {
    ...
    try {
      ...
      console.log("AuthPage: Resetting loading state after email auth");
      setLoading(false);
    }
  };
          ...
          <div className="flex-grow flex justify-center items-center pt-20">
            <div className="bg-white p-8 rounded-xl shadow-lg w-full max-w-md text-center min-h-[72vh]">
              <div className="flex bg-gray-200 rounded-lg overflow-hidden mb-6">
                ...
                <button
                  className={`flex-1 py-2 text-lg font-semibold transition-colors ${!isLogin ? "bg-[#0B2273] text-white" : "text-gray-600"}`}
                  onClick={() => setствиемIsLogin(false)}
                  disabled={loading}
                >
                  Signup
                </button>
